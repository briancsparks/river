...
var DataSet = function(options_) {
  ...
  if (options.destKey)      { self.destKey    = options.destKey; }
  if (options.requestId)    { self.requestId  = options.requestId; }
  ...
  self.sendTo = function(destKey_, requestId_, callback_) {
    ...
    return redis.lpush(`river:feed:${destKey}`, JSON.stringify(data), function(err, receipt) {
