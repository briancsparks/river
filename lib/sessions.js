...
lib.sendSessionsToFeed = function() {
  ...
  var ra = raLib.adapt(arguments, (argv, context, callback) => {
    ...
    var   optionsForResponse  = {destKey, dataType: 'dbRecords'};

    setOnn(optionsForResponse, 'requestId', requestId);
    ...
    return bootstrap('sendSessionsToFeed', callback, function(err, db, config, eabort, abort) {
      ...
      return cursor.toArray(eabort(function(err, receipt) {
        ...
        var dataSet = new DataSet({name:'sessions', items:prepForHttp(receipt)}, optionsForResponse);
...
lib.sendSessionDataToFeed = function() {
  ...
  var ra = raLib.adapt(arguments, (argv, context, callback) => {
    ...
    var   optionsForResponse  = {destKey, dataTypes};

    setOnn(optionsForResponse, 'requestId',     requestId);
    setOnn(optionsForResponse, 'asTimeSeries',  asTimeSeries);
    ...
    return sg.iwrap('sendSessionDataToFeed', callback, function(eabort) {
      return s3.listObjectsV2({Bucket, Prefix}, eabort(function(err, data) {
        ...
        return sg.__eachll(s3objects.Contents, function(contents, nextContent)  {
          ...
          //console.log(`Making request for ${contents.Key}`);

          var dataPoints;
          var dataSet = new DataSet({}, optionsForResponse);
          return sg.__run3([function(next, enext, enag, ewarn) {
            ...
            return s3.getObject({Bucket, Key:contents.Key}, function(err, s3file) {
              //console.log(`S3 response for ${contents.Key}`, {err});
              ...
              // If the request does not want this type, stop processing now
              if (dataTypes) {
                if (dataPoints.dataType && !dataTypes[dataPoints.dataType]) {
                  console.log(`Skipping ${dataPoints.dataType}, not ${_.keys(dataTypes).join(',')}`);
                  return nextContent();
                }
              }
            ...
            dataSet.pushItems('dataPoints', prepForHttp(dataPoints));
            return next();
          }], function() {
            //console.log(`Sending to redis ${contents.Key}`);
            return dataSet.sendTo(function(err, receipt) {
              //console.log(`Redis response for redis ${contents.Key}`, {err});
              return nextContent();
            });
          });
