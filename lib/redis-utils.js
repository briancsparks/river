...
lib.tellStackService = function(name, location, ttl, stack, callback) {

  // Get the IP from location
  const m = /([0-9]+[.][0-9]+[.][0-9]+[.][0-9]+)/.exec(location);

  const ip            = (m || [])[1] || 'notanip';
  const serviceName   = _.compact(['service', stack, name, lib.getHour()]).join(':');
...
lib.tellService = function(name, location, ttl, callback) {
  return lib.tellStackService(name, location, ttl, null, callback);
};
